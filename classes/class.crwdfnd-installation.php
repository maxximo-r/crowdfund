...
class CrwdfndInstallation {
        ...
        if (function_exists('is_multisite') && is_multisite()) {
            ...
            if (isset($_GET['networkwide']) && ($_GET['networkwide'] == 1)) {
                ...
                foreach ($blogids as $blog_id) {
                    ...
                    CrwdfndInstallation::installer();
                    CrwdfndInstallation::initdb();
                }
    ...
    public static function installer() {
        ...
        $sql = "CREATE TABLE " . $wpdb->prefix . "crwdfnd_members_tbl (
        ...
        $sql = "CREATE TABLE " . $wpdb->prefix . "crwdfnd_membership_tbl (
        ...
        dbDelta($sql);
        $sql = "SELECT * FROM " . $wpdb->prefix . "crwdfnd_membership_tbl WHERE id = 1";
        $results = $wpdb->get_row($sql);
        if (is_null($results)) {
            $sql = "INSERT INTO  " . $wpdb->prefix . "crwdfnd_membership_tbl  (
			id ,
			alias ,
			role ,
			permissions ,
        ...
        $sql = "UPDATE  " . $wpdb->prefix . "crwdfnd_membership_tbl SET subscription_duration_type = 1 WHERE subscription_unit='days' AND subscription_duration_type = 0";
        $wpdb->query($sql);

        $sql = "UPDATE  " . $wpdb->prefix . "crwdfnd_membership_tbl SET subscription_duration_type = 2 WHERE subscription_unit='weeks' AND subscription_duration_type = 0";
        $wpdb->query($sql);

        $sql = "UPDATE  " . $wpdb->prefix . "crwdfnd_membership_tbl SET subscription_duration_type = 3 WHERE subscription_unit='months' AND subscription_duration_type = 0";
        $wpdb->query($sql);

        $sql = "UPDATE  " . $wpdb->prefix . "crwdfnd_membership_tbl SET subscription_duration_type = 4 WHERE subscription_unit='years' AND subscription_duration_type = 0";
        $wpdb->query($sql);

        $sql = "CREATE TABLE " . $wpdb->prefix . "crwdfnd_membership_meta_tbl (
        ...
        $sql = "CREATE TABLE " . $wpdb->prefix . "crwdfnd_payments_tbl (
        ...
        update_option("crwdfnd_db_version", CROWDFUND_ME_DB_VER);
    }

    public static function initdb() {
        $settings = CrwdfndSettings::get_instance();

        $installed_version = $settings->get_value('crwdfnd-active-version');
        ...
        if (empty($installed_version)) {
            ...
            $example_from_address = 'hello@' . CrwdfndMiscUtils::get_home_url_without_http_and_www();
        ...
        if (version_compare($installed_version, CROWDFUND_ME_VER) == -1) {
            //Do upgrade tasks
        }

        $settings->set_value('crwdfnd-active-version', CROWDFUND_ME_VER)->save(); //save everything.
        //Generate and save a crwdfnd private key for this site
        $unique_id = uniqid('', true);
        add_option('crwdfnd_private_key_one', $unique_id);
    }

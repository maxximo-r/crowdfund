...
class CrwdfndLevelForm {
    ...
    protected function subscription_period() {
        ...
        if ($subscript_duration_type == CrwdfndMembershipLevel::NO_EXPIRY) {
        ...
        if (($subscript_duration_type == CrwdfndMembershipLevel::FIXED_DATE)){
            $dateinfo = date_parse($subscription_period);
            if ($dateinfo['warning_count']|| $dateinfo['error_count']){
                $this->errors['subscription_period'] = CrwdfndUtils::_("Date format is not valid.");
                return;
            }
        ...
        if (!is_numeric($subscription_period)) {
            $this->errors['subscription_period'] = CrwdfndUtils::_("Access duration must be > 0.");
            return;
        }
        $this->sanitized['subscription_period'] = sanitize_text_field($subscription_period);
    }

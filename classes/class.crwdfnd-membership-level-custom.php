 ...
 * @author maxximo
 */
class CrwdfndMembershipLevelCustom {
    ...
    public static function get_instance_by_id($level_id){
        if (!isset(self::$instances[$level_id])){
            self::$instances[$level_id] = new CrwdfndMembershipLevelCustom();
    ...
    public function load_by_id($level_id){
        global $wpdb;
        $query = 'SELECT * FROM ' . $wpdb->prefix . 'crwdfnd_membership_meta_tbl WHERE level_id=%d';
    ...
    private function save($field){
        ...
        $query = $wpdb->prepare(
                'REPLACE INTO ' . $wpdb->prefix. 'crwdfnd_membership_meta_tbl
    ...
    public static function get_value_by_key($level_id, $key, $default= ''){
        return CrwdfndMembershipLevelCustom::get_instance_by_id($level_id)->get($key, $default);
    }
    public static  function get_value_by_context($level_id, $context){
        return CrwdfndMembershipLevelCustom::get_instance_by_id($level_id)->get_by_context($context);
    }
}

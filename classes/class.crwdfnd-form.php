...
class CrwdfndForm {
    ...
    protected function validate_wp_user_email(){
        ...
        if ($user && ($user->user_email != $email)){
            $error_msg = CrwdfndUtils::_("Wordpress account exists with given username. But the given email doesn't match.");
            $error_msg .= CrwdfndUtils::_(" Use a different username to complete the registration. If you want to use that username then you must enter the correct email address associated with the existing WP user to connect with that account.");
            $this->errors['wp_email'] = $error_msg;
            return;
        }
        $user = get_user_by('email', $email);
        if($user && ($user_name != $user->user_login)){
            $error_msg = CrwdfndUtils::_("Wordpress account exists with given email. But the given username doesn't match.");
            $error_msg .= CrwdfndUtils::_(" Use a different email address to complete the registration. If you want to use that email then you must enter the correct username associated with the existing WP user to connect with that account.");
    ...
    protected function user_name() {
        ...
        $user_name = isset($_POST['user_name']) ? sanitize_text_field($_POST['user_name']) : '';
        if (empty($user_name)) {
            $this->errors['user_name'] = CrwdfndUtils::_('Username is required');
            return;
        }
        if (!CrwdfndMemberUtils::is_valid_user_name($user_name)) {
            $this->errors['user_name'] = CrwdfndUtils::_('Username contains invalid character');
            return;
        }
        $saned = sanitize_text_field($user_name);
        $query = "SELECT count(member_id) FROM {$wpdb->prefix}crwdfnd_members_tbl WHERE user_name= %s";
        $result = $wpdb->get_var($wpdb->prepare($query, strip_tags($saned)));
        if ($result > 0) {
            if ($saned != $this->fields['user_name']) {
                $this->errors['user_name'] = CrwdfndUtils::_('Username already exists.');
                return;
            }
        }
    ...
    protected function password() {
        ...
        if (empty($this->fields['password']) && empty($password)) {
            $this->errors['password'] = CrwdfndUtils::_('Password is required');
            return;
        }
        if (!empty($password)) {
            ...
            if ($saned != $saned_re){
                $this->errors['password'] = CrwdfndUtils::_('Password mismatch');
            }
            $this->sanitized['plain_password'] = $password;
            $this->sanitized['password'] = CrwdfndUtils::encrypt_password(trim($password)); //should use $saned??;
        }
    }
    ...
    protected function email() {
        ...
        $email = filter_input(INPUT_POST, 'email', FILTER_UNSAFE_RAW);
        if (empty($email)) {
            $this->errors['email'] = CrwdfndUtils::_('Email is required');
            return;
        }
        if (!is_email($email)) {
            $this->errors['email'] = CrwdfndUtils::_('Email is invalid') . " (".esc_attr($email).")";
            return;
        }
        $saned = sanitize_email($email);
        $query = "SELECT count(member_id) FROM {$wpdb->prefix}crwdfnd_members_tbl WHERE email= %s";
        ...
        if ($result > 0) {
            if ($saned != $this->fields['email']) {
                $error_msg = CrwdfndUtils::_('Email is already used.') . " (" . $saned .")";
    ...
    protected function member_since() {
        ...
        $this->errors['member_since'] = CrwdfndUtils::_('Member since field is invalid');
    ...
    protected function subscription_starts() {
        ...
        $this->errors['subscription_starts'] = CrwdfndUtils::_('Access starts field is invalid');
    }
    ...
    protected function gender() {
        ...
        if (in_array($gender, array('male', 'female', 'not specified'))){
            ...
            $this->errors['gender'] = CrwdfndUtils::_('Gender field is invalid');
        }
    }
    ...
    protected function account_state() {
        ...
        if (in_array($account_state, array('active', 'pending', 'activation_required', 'inactive', 'expired'))){
            ...
            $this->errors['account_state'] = CrwdfndUtils::_('Account state field is invalid');
        }
    }
    ...
    protected function membership_level() {
        ...
        if ($membership_level == 1){
            $this->errors['membership_level'] = CrwdfndUtils::_('Invalid membership level');
            return;
        }

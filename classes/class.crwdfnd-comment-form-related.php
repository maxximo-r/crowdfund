...
class CrwdfndCommentFormRelated {

    public static function customize_comment_form() {
        $allow_comments = CrwdfndSettings::get_instance()->get_value('members-login-to-comment');
        if (empty($allow_comments)){
            return;
        }
        if (CrwdfndAuth::get_instance()->is_logged_in()){
            return;
        }

        //Apply a filter to the message so it can be customized using the custom message plugin
        $comment_form_msg = apply_filters('crwdfnd_login_to_comment_msg', CrwdfndUtils::_("Please login to comment."));
        $comment_form_msg = '<div class="crwdfnd-login-to-comment-msg">' . $comment_form_msg . '</div>';
        ?>
    ...
    public static function customize_comment_fields($fields){
        ...
        if (CrwdfndAuth::get_instance()->is_logged_in()){//Member is logged-in.
            return $fields;
        }

        //Member is not logged-in so show the protection message.
        $fields = array();
        $login_link = CrwdfndUtils::_('Please Login to Comment.');
    ...
    public static function check_and_restrict_comment_posting_to_members(){
        ...
        $_POST = array();
        wp_die(CrwdfndUtils::_('Comments not allowed by a non-member.'));
    }

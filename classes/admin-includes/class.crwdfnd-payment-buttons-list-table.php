...
class CrwdfndPaymentButtonsListTable extends WP_List_Table {
    ...
    function column_default($item, $column_name) {
        ...
        switch ($column_name) {
            ...
            case 'button_type':
                ...
                $button_name=CrwdfndMiscUtils::get_button_type_name($button_type);
                return $button_name;
                break;
            case 'button_shortcode':
                $level_id = get_post_meta($item['ID'], 'membership_level_id', true);
                if(!CrwdfndUtils::membership_level_id_exists($level_id)){
                    ...
                    $shortcode = '[crwdfnd_payment_button id='.$item['ID'].']';
                }
                return $shortcode;
                break;
        }
    ...
    function column_ID($item) {
        ...
        //Build row actions
        $actions = array(
            'edit' => sprintf('<a href="admin.php?page=crowdfund_me_payments&tab=edit_button&button_id=%s&button_type=%s">Edit</a>', $item['ID'], $button_type),
            'delete' => sprintf( '<a href="admin.php?page=crowdfund_me_payments&tab=payment_buttons&action=delete_payment_btn&button_id=%s&_wpnonce=%s" onclick="return confirm(\'Are you sure you want to delete this record?\')">Delete</a>', $item['ID'], wp_create_nonce( 'crwdfnd_delete_payment_btn_' . $item['ID'] ) ),
        );
    ...
    function get_columns() {
        $columns = array(
            ...
            'ID' => CrwdfndUtils::_('Payment Button ID'),
            'title' => CrwdfndUtils::_('Payment Button Title'),
            'membership_level' => CrwdfndUtils::_('Membership Level ID'),
            'button_type' => CrwdfndUtils::_('Button Type'),
            'button_shortcode' => CrwdfndUtils::_('Button Shortcode'),
        );
        return $columns;
    }
...
//        );
    ...
    function process_delete_action() {
        ...
        if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'delete_payment_btn') { //Delete link was clicked for a row in list table
            ...
            check_admin_referer( 'crwdfnd_delete_payment_btn_' . $record_id );
            wp_delete_post( $record_id );
    ...
    function get_total_items() {
        $counts = wp_count_posts('crwdfnd_payment_button');
        $total = 0;

...
class CrwdfndPostList extends WP_List_Table {
        ...
        switch ($this->type) {
            case 'page':
                return array(
                    ...
                    , 'date' => CrwdfndUtils::_('Date')
                    , 'title' => CrwdfndUtils::_('Title')
                    , 'author' => CrwdfndUtils::_('Author')
                    , 'status' => CrwdfndUtils::_('Status')
                );
                break;
            case 'post':
                return array(
                    ...
                    , 'title' => CrwdfndUtils::_('Title')
                    , 'author' => CrwdfndUtils::_('Author')
                    , 'categories' => CrwdfndUtils::_('Categories')
                    , 'status' => CrwdfndUtils::_('Status')
                );
                break;
            case 'custom_post':
    ...
    public static function update_post_list() {
        ...
        CrwdfndMiscUtils::check_user_permission_and_is_admin('post protection update');

        //Check nonce
        $crwdfnd_post_prot_update_nonce = filter_input(INPUT_POST, 'crwdfnd_post_prot_update_nonce');
        if (!wp_verify_nonce($crwdfnd_post_prot_update_nonce, 'crwdfnd_post_prot_update_nonce_action')) {
            //Nonce check failed.
            wp_die(CrwdfndUtils::_("Error! Nonce security verification failed for Post Protection Update action. Clear cache and try again."));
        }
        ...
        $message = array('succeeded' => true, 'message' => '<p class="crwdfnd-green-box">' . CrwdfndUtils::_('Protection settings updated!') . '</p>');

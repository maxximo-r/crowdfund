 ...
 * @author maxximo
 */
class CrwdfndCronJob {

    public function __construct() {
        add_action('crwdfnd_account_status_event', array(&$this, 'update_account_status'));
        add_action('crwdfnd_delete_pending_account_event', array(&$this, 'delete_pending_account'));
        add_action('crwdfnd_delete_pending_account_event', array($this, 'delete_pending_email_activation_data'));
    }
    ...
    public function update_account_status() {
        ...
        for ($counter = 0;; $counter += 100) {
            $query = $wpdb->prepare("SELECT member_id, membership_level, subscription_starts, account_state
                    FROM {$wpdb->prefix}crwdfnd_members_tbl
                    WHERE membership_level NOT IN ( SELECT id FROM {$wpdb->prefix}crwdfnd_membership_tbl
            ...
            foreach ($results as $result) {
                $timestamp = CrwdfndUtils::get_expiration_timestamp($result);
    ...
    public function delete_pending_account() {
        global $wpdb;
        $interval = CrwdfndSettings::get_instance()->get_value('delete-pending-account');
        if (empty($interval)) {
            return;
        }
        for ($counter = 0;; $counter += 100) {
            ...
            if (count($to_delete) > 0) {
                CrwdfndLog::log_simple_debug("Auto deleting pending account.", true);
                $query = "DELETE FROM {$wpdb->prefix}crwdfnd_members_tbl
    ...
    public function delete_pending_email_activation_data() {
        ...
        global $wpdb;
        $q = "SELECT * FROM {$wpdb->prefix}options WHERE option_name LIKE '%crwdfnd_email_activation_data_usr_%'";

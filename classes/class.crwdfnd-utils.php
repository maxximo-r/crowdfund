...
abstract class CrwdfndUtils {
	...
	public static function do_misc_initial_plugin_setup_tasks() {
		...
		$admin_dashboard_permission = CrwdfndSettings::get_instance()->get_value( 'admin-dashboard-access-permission' );
		if ( empty( $admin_dashboard_permission ) ) {
			//By default only admins can manage/see admin dashboard
			define( 'CRWDFND_MANAGEMENT_PERMISSION', 'manage_options' );
		} else {
			define( 'CRWDFND_MANAGEMENT_PERMISSION', $admin_dashboard_permission );
		}
                ...
                add_filter( 'option_page_capability_crwdfnd-settings-tab-1', 'CrwdfndUtils::crwdfnd_settings_update_capability' );
                add_filter( 'option_page_capability_crwdfnd-settings-tab-2', 'CrwdfndUtils::crwdfnd_settings_update_capability' );
                add_filter( 'option_page_capability_crwdfnd-settings-tab-3', 'CrwdfndUtils::crwdfnd_settings_update_capability' );
                add_filter( 'option_page_capability_crwdfnd-settings-tab-4', 'CrwdfndUtils::crwdfnd_settings_update_capability' );
                add_filter( 'option_page_capability_crwdfnd-settings-tab-5', 'CrwdfndUtils::crwdfnd_settings_update_capability' );

	}

        public static function crwdfnd_settings_update_capability($capability){
            if ( defined('CRWDFND_MANAGEMENT_PERMISSION') ){
                //Use CRWDFND defined one.
                $capability = CRWDFND_MANAGEMENT_PERMISSION;
            } else {
                //Use default.
	...
	public static function subscription_type_dropdown( $selected ) {
		return '<option ' . ( ( $selected == CrwdfndMembershipLevel::NO_EXPIRY ) ? 'selected="selected"' : '' ) . ' value="' . CrwdfndMembershipLevel::NO_EXPIRY . '">No Expiry</option>' .
				'<option ' . ( ( $selected == CrwdfndMembershipLevel::DAYS ) ? 'selected="selected"' : '' ) . ' value="' . CrwdfndMembershipLevel::DAYS . '">Day(s)</option>' .
				'<option ' . ( ( $selected == CrwdfndMembershipLevel::WEEKS ) ? 'selected="selected"' : '' ) . ' value="' . CrwdfndMembershipLevel::WEEKS . '">Week(s)</option>' .
				'<option ' . ( ( $selected == CrwdfndMembershipLevel::MONTHS ) ? 'selected="selected"' : '' ) . ' value="' . CrwdfndMembershipLevel::MONTHS . '">Month(s)</option>' .
				'<option ' . ( ( $selected == CrwdfndMembershipLevel::YEARS ) ? 'selected="selected"' : '' ) . ' value="' . CrwdfndMembershipLevel::YEARS . '">Year(s)</option>' .
				'<option ' . ( ( $selected == CrwdfndMembershipLevel::FIXED_DATE ) ? 'selected="selected"' : '' ) . ' value="' . CrwdfndMembershipLevel::FIXED_DATE . '">Fixed Date</option>';
	}
	...
	public static function calculate_subscription_period_days( $subcript_period, $subscription_duration_type ) {
		if ( $subscription_duration_type == CrwdfndMembershipLevel::NO_EXPIRY ) {
			return 'noexpire';
		}
		if ( ! is_numeric( $subcript_period ) ) {
			throw new Exception( ' subcript_period parameter must be integer in CrwdfndUtils::calculate_subscription_period_days method' );
		}
		switch ( strtolower( $subscription_duration_type ) ) {
			case CrwdfndMembershipLevel::DAYS:
				break;
			case CrwdfndMembershipLevel::WEEKS:
				$subcript_period = $subcript_period * 7;
				break;
			case CrwdfndMembershipLevel::MONTHS:
				$subcript_period = $subcript_period * 30;
				break;
			case CrwdfndMembershipLevel::YEARS:
	...
	public static function get_expiration_timestamp( $user ) {
		$permission = CrwdfndPermission::get_instance( $user->membership_level );
		if ( CrwdfndMembershipLevel::FIXED_DATE == $permission->get( 'subscription_duration_type' ) ) {
	...
	public static function get_formatted_expiry_date( $start_date, $subscription_duration, $subscription_duration_type ) {
		if ( $subscription_duration_type == CrwdfndMembershipLevel::FIXED_DATE ) {
	...
	public static function gender_dropdown( $selected = 'not specified' ) {
		return '<option ' . ( ( strtolower( $selected ) == 'male' ) ? 'selected="selected"' : '' ) . ' value="male">' . CrwdfndUtils::_('Male') . '</option>' .
				'<option ' . ( ( strtolower( $selected ) == 'female' ) ? 'selected="selected"' : '' ) . ' value="female">' . CrwdfndUtils::_('Female') . '</option>' .
				'<option ' . ( ( strtolower( $selected ) == 'not specified' ) ? 'selected="selected"' : '' ) . ' value="not specified">' . CrwdfndUtils::_('Not Specified') . '</option>';
	}
	...
	public static function membership_level_dropdown( $selected = 0 ) {
		$options = '';
		global $wpdb;
		$query  = 'SELECT alias, id FROM ' . $wpdb->prefix . 'crwdfnd_membership_tbl WHERE id != 1';
	...
	public static function get_membership_level_row_by_id( $level_id ) {
		global $wpdb;
		$query           = $wpdb->prepare( 'SELECT * FROM ' . $wpdb->prefix . 'crwdfnd_membership_tbl WHERE id=%d', $level_id );
	...
	public static function get_registration_complete_prompt_link( $for = 'all', $send_email = false, $member_id = '' ) {
		...
		switch ( $for ) {
			case 'one':
				...
				$query   = $wpdb->prepare( "SELECT * FROM  {$wpdb->prefix}crwdfnd_members_tbl WHERE member_id =  %d", $member_id );
				$members = $wpdb->get_results( $query );
				break;
			case 'all':
				$query   = "SELECT * FROM  {$wpdb->prefix}crwdfnd_members_tbl WHERE reg_code != '' ";
				$members = $wpdb->get_results( $query );
				break;
		}
		...
		foreach ( $members as $member ) {
			...
			if ( $send_email && empty( $member->user_name ) ) {
				...
				$subject    = apply_filters( 'crwdfnd_email_complete_your_registration_subject', $subject );
				$email_body = apply_filters( 'crwdfnd_email_complete_your_registration_body', $email_body );
				CrwdfndMiscUtils::mail( $member->email, $subject, $email_body, $headers );
				CrwdfndLog::log_simple_debug( 'Prompt to complete registration email sent to: ' . $member->email . '. From email address value used: ' . $from_address, true );
			}
			$links[] = $reg_url;
		}
		return $links;
	...
	/* This function is deprecated and will be removed in the future. Use CrwdfndMemberUtils::update_wp_user_role() instead */

	public static function update_wp_user_Role( $wp_user_id, $role ) {
		// Deprecated function.
		CrwdfndMemberUtils::update_wp_user_role( $wp_user_id, $role );
	}

	public static function update_wp_user( $wp_user_name, $crwdfnd_data ) {
		$wp_user_info = array();
		if ( isset( $crwdfnd_data['email'] ) ) {
			$wp_user_info['user_email'] = $crwdfnd_data['email'];
		}
		if ( isset( $crwdfnd_data['first_name'] ) ) {
			$wp_user_info['first_name'] = $crwdfnd_data['first_name'];
		}
		if ( isset( $crwdfnd_data['last_name'] ) ) {
			$wp_user_info['last_name'] = $crwdfnd_data['last_name'];
		}
		if ( isset( $crwdfnd_data['plain_password'] ) ) {
			$wp_user_info['user_pass'] = $crwdfnd_data['plain_password'];
		}
	...
	public static function create_wp_user( $wp_user_data ) {
                ...
                CrwdfndMemberUtils::check_and_die_if_email_belongs_to_admin_user($wp_user_data['user_email']);
                CrwdfndMemberUtils::check_and_die_if_username_belongs_to_admin_user($wp_user_data['user_login']);
		...
		//Lets create a new wp user record or attach the CRWDFND profile to an existing user accordingly.
	...
	/*
	 * Deprecated. Instead use CrwdfndUtils::has_admin_management_permission()
	 */
	...
	public static function crwdfnd_username_exists( $user_name ) {
		global $wpdb;
		$member_table = $wpdb->prefix . 'crwdfnd_members_tbl';
	...
	public static function get_free_level() {
		...
		if ( ! empty( $encrypted ) ) {
                    ...
                    if ( $hash_val != $encrypted ) {//level_identifier validation failed.
                            ...
                            if ( isset ( $_POST['crwdfnd-fb-submit'] ) ){//Form builder submission potentially
                                $msg .= '<p>If you are using the Form Builder addon, please update the addon and try again.</p>';
                            }
                            wp_die( $msg );
                    }

                    return CrwdfndPermission::get_instance( $encrypted )->get( 'id' );
		}
	...
	public static function get_paid_member_info() {
		...
		global $wpdb;
		if ( ! empty( $member_id ) && ! empty( $code ) ) {
			$query = 'SELECT * FROM ' . $wpdb->prefix . 'crwdfnd_members_tbl WHERE member_id= %d AND reg_code=%s';
	...
	public static function get_incomplete_paid_member_info_by_ip() {
		...
		if ( ! empty( $user_ip ) ) {
			...
			$username = '';
			$query    = 'SELECT * FROM ' . $wpdb->prefix . 'crwdfnd_members_tbl WHERE last_accessed_from_ip=%s AND user_name=%s';
	...
	public static function account_delete_confirmation_ui( $msg = '' ) {
		ob_start();
		include CROWDFUND_ME_PATH . 'views/account_delete_warning.php';
		ob_get_flush();
		wp_die( '', '', array( 'back_link' => true ) );
	}

	public static function delete_account_button() {
		$allow_account_deletion = CrwdfndSettings::get_instance()->get_value( 'allow-account-deletion' );
		if ( empty( $allow_account_deletion ) ) {
			return '';
		}

		$account_delete_link  = '<div class="crwdfnd-profile-account-delete-section">';
		$account_delete_link .= '<a href="' . CROWDFUND_ME_SITE_HOME_URL . '/?crwdfnd_delete_account=1"><div class="crwdfnd-account-delete-button">' . self::_( 'Delete Account' ) . '</div></a>';
	...
	public static function get_restricted_image_url() {
		return CROWDFUND_ME_URL . '/images/restricted-icon.png';
	}
	...
	public static function get_user_ip_address() {
		...
		return apply_filters( 'crwdfnd_get_user_ip_address', $user_ip );
	}

	public static function is_first_click_free( &$content ) {
		$is_first_click                 = false;
		$args                           = array( $is_first_click, $content );
		$filtered                       = apply_filters( 'crwdfnd_first_click_free', $args );

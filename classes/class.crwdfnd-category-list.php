...
class CrwdfndCategoryList extends WP_List_Table {
    ...
    function __construct() {
        ...
        $this->category = ($this->selected_level_id == 1) ?
                CrwdfndProtection::get_instance() :
                CrwdfndPermission::get_instance($this->selected_level_id);
    }

    function get_columns() {
        return array(
            'cb' => '<input type="checkbox" />'
            , 'term_id' => CrwdfndUtils::_('Category ID')
            , 'name' => CrwdfndUtils::_('Category Name')
            , 'taxonomy' => CrwdfndUtils::_('Category Type (Taxonomy)')
            , 'description' => CrwdfndUtils::_('Description')
            , 'count' => CrwdfndUtils::_('Count')
        );
    }
    ...
    public static function update_category_list() {
        ...
        CrwdfndMiscUtils::check_user_permission_and_is_admin('category protection update');

        //Check nonce
        $crwdfnd_category_prot_update_nonce = filter_input(INPUT_POST, 'crwdfnd_category_prot_update_nonce');
        if (!wp_verify_nonce($crwdfnd_category_prot_update_nonce, 'crwdfnd_category_prot_update_nonce_action')) {
            //Nonce check failed.
            wp_die(CrwdfndUtils::_("Error! Nonce security verification failed for Category Protection Update action. Clear cache and try again."));
        }
        ...
        $category = ($selected_level_id == 1) ?
                CrwdfndProtection::get_instance() :
                CrwdfndPermission::get_instance($selected_level_id);
        ...
        $message = array('succeeded' => true, 'message' => '<p class="crwdfnd-green-box">' . CrwdfndUtils::_('Category protection updated!') . '</p>');
    ...
    function no_items() {
        CrwdfndUtils::e('No category found.');
    }

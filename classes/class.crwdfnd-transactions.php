...
class CrwdfndTransactions {

	static function save_txn_record( $ipn_data, $items = array() ) {
		global $wpdb;

		$current_date = CrwdfndUtils::get_current_date_in_wp_zone();//date( 'Y-m-d' );
		...
		$txn_data['member_id']        = isset ( $custom_var['crwdfnd_id'] ) ? $custom_var['crwdfnd_id'] : '';
		...
		$db_row_id = $wpdb->insert_id;
                ...
                /*** Save to the crwdfnd_transactions CPT also ***/
		//Let's also store the transactions data in crwdfnd_transactions CPT.
		$post                = array();
		$post['post_title']  = '';
		...
		update_post_meta( $post_id, 'db_row_id', $db_row_id );
                ...
                //Add the payment_button_id to the txn_data array so it can be saved to the crwdfnd_transactions CPT.
		if ( isset( $ipn_data['payment_button_id'] ) ) {
			$txn_data['payment_button_id'] = $ipn_data['payment_button_id'];
		}

                //Add the is_live to the txn_data array so it can be saved to the crwdfnd_transactions CPT.
		if ( isset( $ipn_data['is_live'] ) ) {
			$txn_data['is_live'] = $ipn_data['is_live'];
		}

                //Add the custom value to the txn_data array so it can be saved to the crwdfnd_transactions CPT.
		if ( isset( $ipn_data['custom'] ) ) {
			$txn_data['custom'] = $ipn_data['custom'];
		}

                //Save the $txn_data to the crwdfnd_transactions CPT as post meta.
		foreach ( $txn_data as $key => $value ) {
			update_post_meta( $post_id, $key, $value );
		}

                //Trigger the action hook.
		do_action( 'crwdfnd_txn_record_saved', $txn_data, $db_row_id, $post_id );
	...
	static function parse_custom_var( $custom ) {
        ...
        static function get_transaction_row_by_subscr_id ($subscr_id) {
                global $wpdb;
                $query_db = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}crwdfnd_payments_tbl WHERE subscr_id = %s", $subscr_id ), OBJECT );
                return $query_db;
        }

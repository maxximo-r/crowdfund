...
class crwdfnd_paypal_ipn_handler {
	...
	public function crwdfnd_validate_and_create_membership() {
		...
		global $wpdb;
		$txn_row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}crwdfnd_payments_tbl WHERE txn_id = %s", $txn_id ), OBJECT );
		...
		foreach ( $cart_items as $current_cart_item ) {
			...
			if ( $check_price && ! $pp_hosted_button ) {
				...
				if ( $button_type == 'pp_buy_now' ) {// This is a PayPal buy now type button
				...
				} elseif ( $button_type == 'pp_subscription' ) {
					...
					$billing_amount = get_post_meta( $button_id, 'billing_amount', true );
                                        ...
                                        if ( empty( $trial_billing_cycle ) ){
                                            ...
                                            if ( crwdfnd_is_paypal_recurring_payment($this->ipn_data) ){
                                                $expected_amount = round( $billing_amount, 2 );
                                            } else {
			...
			if ( ! empty( $subsc_ref ) ) {
				...
				if ( $transaction_type == 'web_accept' ) {
					...
					crwdfnd_update_member_subscription_start_date_if_applicable( $this->ipn_data );
				}
			} else {
		...
		do_action( 'crwdfnd_paypal_ipn_processed', $this->ipn_data );
	...
	public function crwdfnd_validate_ipn() {
		$post_string = '';
	...
	public function validate_ipn_using_remote_post() {
		...
		$params = array(
			...
			'compress'    => false,
			'decompress'  => false,
			'user-agent'  => 'CrowdFund Me Plugin',
		);
...
$ipn_handler_instance = new crwdfnd_paypal_ipn_handler();
...
// Validate the IPN
if ( $ipn_handler_instance->crwdfnd_validate_ipn() ) {

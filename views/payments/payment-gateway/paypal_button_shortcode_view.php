 ...
 * *********************************************** */
add_filter('crwdfnd_payment_button_shortcode_for_pp_buy_now', 'crwdfnd_render_pp_buy_now_button_sc_output', 10, 2);

function crwdfnd_render_pp_buy_now_button_sc_output($button_code, $args) {

    $button_id = isset($args['id']) ? $args['id'] : '';
    if (empty($button_id)) {
        return '<p class="crwdfnd-red-box">Error! crwdfnd_render_pp_buy_now_button_sc_output() function requires the button ID value to be passed to it.</p>';
    }
    ...
    $notify_url = CROWDFUND_ME_SITE_HOME_URL . '/?crwdfnd_process_ipn=1';
    $return_url = get_post_meta($button_id, 'return_url', true);
    if (empty($return_url)) {
        $return_url = CROWDFUND_ME_SITE_HOME_URL;
    }
    $cancel_url = CROWDFUND_ME_SITE_HOME_URL;
    ...
    if (CrwdfndMemberUtils::is_member_logged_in()) {
        $custom_field_value .= '&crwdfnd_id=' . CrwdfndMemberUtils::get_logged_in_members_id();
    }
    $custom_field_value = apply_filters('crwdfnd_custom_field_value_filter', $custom_field_value);

    /* === PayPal Buy Now Button Form === */
    $output = '';
    $output .= '<div class="crwdfnd-button-wrapper crwdfnd-pp-buy-now-wrapper">';
    $uniqid=uniqid();
    ...
    $output.=sprintf('<form id="crwdfnd-paypal-payment-form-%s" action="%s" method="post" %s>',$uniqid,$action_url,$window_target);

    $output .= '<input type="hidden" name="cmd" value="_xclick" />';
    $output .= '<input type="hidden" name="charset" value="utf-8" />';
    $output .= '<input type="hidden" name="bn" value="TipsandTricks_SP" />';
    $output .= '<input type="hidden" name="business" value="' . apply_filters('crwdfnd_buy_now_button_paypal_email',$paypal_email) . '" />';
    ...
    $output .= apply_filters('crwdfnd_pp_payment_form_additional_fields', '');
    
    $button_image_url = get_post_meta($button_id, 'button_image_url', true);
    if (!empty($button_image_url)) {
        $output .= '<input type="image" src="' . $button_image_url . '" class="crwdfnd-buy-now-button-submit" alt="' . CrwdfndUtils::_('Buy Now') . '"/>';
    } else {
        $button_text = (isset($args['button_text'])) ? $args['button_text'] : CrwdfndUtils::_('Buy Now');
        $output .= '<input type="submit" class="crwdfnd-buy-now-button-submit" value="' . $button_text . '" />';
    }

    $output .= '</form>'; //End .form
    $output .= '</div>'; //End .crwdfnd_button_wrapper
 ...
 * *********************************************** */
add_filter('crwdfnd_payment_button_shortcode_for_pp_subscription', 'crwdfnd_render_pp_subscription_button_sc_output', 10, 2);

function crwdfnd_render_pp_subscription_button_sc_output($button_code, $args) {

    $button_id = isset($args['id']) ? $args['id'] : '';
    if (empty($button_id)) {
        return '<p style="color: red;">Error! crwdfnd_render_pp_subscription_button_sc_output() function requires the button ID value to be passed to it.</p>';
    }
    ...
    $output = '';
    $output .= '<div class="crwdfnd-button-wrapper crwdfnd-pp-subscription-wrapper">';
    if ($sandbox_enabled) {
    ...
    $output .= '<input type="hidden" name="charset" value="utf-8" />';
    $output .= '<input type="hidden" name="bn" value="TipsandTricks_SP" />';
    $output .= '<input type="hidden" name="business" value="' . apply_filters('crwdfnd_subscription_button_paypal_email',$paypal_email) . '" />';
    $output .= '<input type="hidden" name="currency_code" value="' . $payment_currency . '" />';
    $output .= '<input type="hidden" name="item_number" value="' . $button_id . '" />';
    $output .= '<input type="hidden" name="item_name" value="' . htmlspecialchars($button_cpt->post_title) . '" />';
    ...
    $button_image_url = get_post_meta($button_id, 'button_image_url', true);
    if (!empty($button_image_url)) {
        $output .= '<input type="image" src="' . $button_image_url . '" class="crwdfnd-subscription-button-submit" alt="' . CrwdfndUtils::_('Subscribe Now') . '"/>';
    } else {
        $button_text = (isset($args['button_text'])) ? $args['button_text'] : CrwdfndUtils::_('Subscribe Now');
        $output .= '<input type="submit" class="crwdfnd-subscription-button-submit" value="' . $button_text . '" />';
    }
